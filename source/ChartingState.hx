...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		var bg = new FlxSprite(0, 0, Paths.image('BG-hank', 'madness'));
		bg.alpha = 0.85;
		bg.scrollFactor.set();
		bg.screenCenter();
		add(bg);
		...
		var tabs = [
			...
			{name: 'Note Types', label: 'Special Note'},
	...
	function addSongUI():Void
	{
		...
		// var types:Array<String> = CoolUtil.coolTextFile(Paths.txt('noteTypesList'));
		// var noteInt:Int = 0;
		...
		// var noteTypeDropDown = new FlxUIDropDownMenu(140, 200, FlxUIDropDownMenu.makeStrIdLabelArray(types, true), function(type:String)
		// 	{
		// 		lastNote.noteType = noteInt;
		// 	});
		// noteTypeDropDown.selectedLabel = types[];
		
		var typeLabel = new FlxText(140,180,64,'Note Type');
		...
		var tab_group_noteTypes = new FlxUI(null, UI_box);
		tab_group_noteTypes.name = "Note Types";


		UI_box.addGroup(tab_group_song);
		UI_box.addGroup(tab_group_assets);
		UI_box.addGroup(tab_group_noteTypes);
		 ...
		 for (i in sectionInfo)
			{
				...
				var daSus = i[2];
				var daType = i[3];
				var note:Note = new Note(daStrumTime, daNoteInfo % 4, null, false, daType, 'madness_Notes');
				if (note.wasGoodHit)
				note.daSkin = 'note_hank-grunt';
				else
				note.daSkin = 'madness_Notes';
