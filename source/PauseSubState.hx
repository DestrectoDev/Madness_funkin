...
class PauseSubState extends MusicBeatSubstate
{
	...
	var menuItems:Array<String> = ['resume', 'restart', 'main menu'];
	...
	public function new(x:Float, y:Float)
	{
		...
		pauseBG = new FlxSprite(0, 0, Paths.image('pauseImages/pauseBG', 'madness'));
		...
		{
			songText = new FlxSprite(0, 50);
			songText.frames = Paths.getSparrowAtlas('pauseImages/pauseItems', 'madness');
			songText.animation.addByPrefix('idle', menuItems[i] + ' buton', 24);
			songText.animation.addByPrefix('select', menuItems[i] + ' select', 24);
			songText.animation.play('idle');
			// songText.isMenuItem = true;
			songText.screenCenter(X);
			songText.x = 200 + (i * 120);
